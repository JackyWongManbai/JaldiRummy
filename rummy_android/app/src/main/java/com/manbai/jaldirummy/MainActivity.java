...
import static com.manbai.jaldirummy.AppUtil.debug;
...
import com.manbai.jaldirummy.PhonePE.PhonePEPay;
...
public class MainActivity extends Activity {
    ...
    private JSONArray upiList = new JSONArray();
    String payUpiList1 = "";
...
//                callBackUrl = msg.getString("callback_url");
    ...
    private JSONObject checkUpdateResult(JSONObject versionData){
        ...
        if(android_force==1 && remote_android_version_code > version_code && version_code>0){//如果服务端要求强制更新到新版本，并且本地是旧版本，那么就必须强制更新Android，跳到应用市场或者下载地址
            ...
            if(remote_android_url.equals("")==true){
...
//        });
    ...
    private void getList() {
        ...
        for (ResolveInfo info: mResolveInfos) {
            ...
            if(UPI.contains(pkgName) && upiList.contains(pkgName)==false){
                upiList.add(pkgName);
            }
        }
    }
...
// merchantUserId - Optional if paymentInstrument.type is: UPI_INTENT, UPI_COLLECT, UPI_QR.
        ...
        data.put("callbackUrl","https://game.jaldirummy.com/api/pay/PhonePECallBack");    //String. Mandatory
